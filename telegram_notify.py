...
def rerating_task_formatter(task):
    ...
    message = (f'<b>task id</b>: {task["task_id"]}\n'
               f'<b>status</b>: {task["task_status"]}\n'
               f'<b>task start time</b>: {task["task_start_time"]}\n'
               f'<b>last update time</b>: {task["task_last_uprate_time"]}\n'                
               f'{src_product}'
               f'{dst_product}'
               f'<b>rerating period</b>: from {task["rerating_start_time"]} till {task["rerating_end_time"]}')
    return message
